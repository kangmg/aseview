...
function getStandardMaterial(color, type) {
    switch(type) {
        case 'glossy': return new THREE.MeshPhongMaterial({ 
            color: color, 
            shininess: 30,  // Reduced from 100 to reduce harsh highlights
            specular: 0x111111,  // Reduced specular
            flatShading: false 
        });
        case 'metallic': return new THREE.MeshStandardMaterial({ 
            color: color, 
            metalness: 0.3, 
            roughness: 0.5,  // Increased from 0.4 for smoother appearance
            flatShading: false 
        });
        default: return new THREE.MeshLambertMaterial({ color: color, flatShading: false });
    }
}
...
function getBondPoints(p1, p2, sym1, sym2, atomScale, style) {
    ...
    // Bonds slightly penetrate into atoms for better visual connection
    // offsetFactor < 1.0 means bond is shorter (doesn't reach atom surface)
    // offsetFactor = 1.0 means bond reaches atom surface
    // offsetFactor > 1.0 means bond penetrates into atom
    let offsetFactor1 = 0.95;  // Slightly shorter for better visibility
    let offsetFactor2 = 0.95;

    // Style-specific adjustments
    if (style === '2d') {
        // 2D style: bonds go all the way to center for flat appearance
        offsetFactor1 = 1.0;
        offsetFactor2 = 1.0;
    } else if (style === 'neon') {
        // Neon style: shorter bonds to avoid overlapping with glow
        offsetFactor1 = 0.85;
        offsetFactor2 = 0.85;
    } else if (style === 'cartoon') {
        // Cartoon style: slight penetration to connect well with outline
        offsetFactor1 = 1.0;
        offsetFactor2 = 1.0;
    } else if (style === 'bubble') {
        // Bubble style: shorter to avoid overlapping with transparent edges
        offsetFactor1 = 0.9;
        offsetFactor2 = 0.9;
    } else if (style === 'glossy' || style === 'metallic' || style === 'default') {
        // Glossy/Metallic/Default: slightly shorter for clean look
        offsetFactor1 = 0.95;
        offsetFactor2 = 0.95;
    }
...
function createAtomStyleCartoon(pos, symbol, atomScale) {
    ...
    const outlineGeometry = new THREE.SphereGeometry(scaledRadius * 1.08, 64, 64);
    ...
    outline.renderOrder = -1;  // Render outline first
